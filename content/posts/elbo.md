...
What’s being optimized: Suppose $q\in \mathcal{Q}$ is generally defined by a probability distribution parameterized by $\theta$. Then we optimize the following quantity
...
→ More importantly, these are not the reasons that we choose the former, but rather a side-effect as a result of being forced to choose $D_{KL}(q\|p)$ for tractability.
